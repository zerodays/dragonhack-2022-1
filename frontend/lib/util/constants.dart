import 'package:flutter/material.dart';

Map<String, Color> allergenColor = {
  "Cereal": Colors.yellowAccent,
  "Crustaceans": Colors.lightGreenAccent,
  "Eggs": Colors.lightGreen,
  "Fish": Colors.blueAccent,
  "Peanuts": Colors.brown,
  "Soybeans": Colors.indigo,
  "Milk": Colors.redAccent,
  "Nuts": Colors.deepPurple,
  "Celery": Colors.blueGrey,
  "Sesame seeds": Colors.green,
  "Sulphur dioxide": Colors.amber,
  "Lupin": Colors.cyan,
  "Molluscs": Colors.purpleAccent
};